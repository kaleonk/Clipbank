...
export default function ClipGrid({
  ...
  async function toggleFavorite(e: React.MouseEvent, clipId: string) {
    ...
    if (favorites.has(clipId)) {
      setFavorites(prev => { const next = new Set(prev); next.delete(clipId); return next })
  ...
  async function convertOne(ffmpeg: FFmpeg, mp4Url: string, title: string, mode: DownloadMode) {
    try {
      ...
      if (!proxyRes.ok) throw new Error(`Proxy failed (${proxyRes.status}): ${await proxyRes.text()}`)
      ...
      if (mode === 'vertical') {
        ...
        if (exitCode !== 0) throw new Error(`FFmpeg copy failed (exit code ${exitCode})`)
      }
  ...
  async function handleDownload(mode: DownloadMode) {
    ...
    try {
      ...
      const ffmpeg = await loadFFmpeg().catch(err => {
      ...
      for (const id of selectedIds) {
        ...
        await convertOne(ffmpeg, mp4Url, clip?.title ?? id, mode)
      }
